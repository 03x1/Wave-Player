...
Frosted glass cards, soft shadows, and colors pulled **live from the album art**. Four colors are lifted from every cover and drift behind the player on their own orbits, so the background moves with the music and stops when you pause.
...
Off by default. Turn it on in Settings for a bar row that follows the track's beat grid.

## Replaces the Spotify Miniplayer

Click Spotify's miniplayer button and Wave Player opens instead. Close it from any mode with the ✕ button. There's also a topbar button and `Ctrl+Shift+M` if you'd rather not use Spotify's.
